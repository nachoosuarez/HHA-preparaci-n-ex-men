% Bombas_paralelo.m — calcula el punto de funcionamiento de DOS bombas
% iguales o distintas conectadas en PARALELO (mismo H, caudales Q1+Q2 se
...
% Salidas: gráfico H-Q (bombas individuales, curva paralelo equivalente,
...
% Usar cuando: dos bombas trabajan en paralelo (se suman los caudales a
...
% Rango total posible del caudal seg�n las curvas reales
Qmin = max([min(Q1), min(Q2)]);

if numel(Q1) == numel(Q2)
    % Misma cantidad de puntos: Qmax = suma de los �ltimos caudales
    Qmax = Q1(end) + Q2(end);
else
    % Distinta cantidad de puntos: Qmax = �ltimo valor del vector m�s largo
...
% GR�FICAS, curvas caracteristicas
...
for i = 1:length(Q)
    ...
    % OJO: se resta vs^2/(2*g) porque HA (mas arriba en este mismo loop)
    % ya es carga TOTAL (piezometrica+cinetica) y ese termino se cancela
    % algebraicamente en la definicion de NPSH (ver nota igual en
    % Bomba_sola.m; bug detectado y corregido resolviendo 2023 dic Ej.4
    % contra la solucion oficial).
    NPSHdisp1 = [NPSHdisp1 10.1 + HA - zB - vs^2/(2*g)];       % bomba 1 curva verde oscuro
    NPSHdisp2 = [NPSHdisp2 10.1 + HA - zB - vs^2/(2*g)]; % bomba 2 curva celeste

endfor
plot(Q, Hm, 'm-', 'LineWidth', 1.2);   % curva de instalaci�n

% Punto de funcionamiento: intersecci�n H_eq(Q) = Hm(Q)
diffH = abs(H_eq - Hm);
[~, idx] = min(diffH);

Qpf = Q(idx);        % caudal en el PF
Hpf = H_eq(idx);     % (o Hm(idx), son pr�cticamente iguales)
...
% L�neas punteadas
% vertical para el caudal total (equivalente + instalaci�n)
...
% l�nea horizontal a la carga de funcionamiento
...
legend("Bomba 1","Bomba 2","Serie","Instalaci�n","Punto de funcionamiento", ...
       "NPSHdisp Bomba 1","NPSHdisp Bomba 2","NPSHr Bomba 1","NPSHr Bomba 2");

%Chequeo de cavitaci�n
...
%---------------------------------------------
% Marcar los puntos de funcionamiento en el gr�fico
%---------------------------------------------
...
% L�neas punteadas para cada bomba, en su propio Q_pf
