% fgv_circ.m — script principal de FLUJO GRADUALMENTE VARIADO (FGV) en
% conducto/canal CIRCULAR (p.ej. caño de alcantarillado que no trabaja a
% presión): calcula yc (crítico, por fzero) y el/los yn (normal(es) —
% en circular puede haber DOS tirantes normales para un mismo caudal si
% y>0.82d aprox., el script chequea Qnmax para saber si existen), integra
% dy/dx (circ.m) con ode23 usando critico_lleno.m como evento de parada
% (cruce de yc, o llegar a sección llena y=d, o y=0), y grafica el
% perfil de la superficie libre junto a las rasantes de fondo, techo
% (y=d), yc y yn. Editar los DATOS DE ENTRADA (Q,d,S,n) y condiciones de
% borde (x_ini,x_end,y_ini) según el enunciado.
% Usar cuando: piden el perfil y(x) de un caño/conducto circular
% funcionando parcialmente lleno. Requiere circ_geom.m, circ.m,
% froude_circ.m, manning_circ.m, critico_lleno.m.
% (El AVISO original menciona "critico.m" pero el script en realidad usa
% critico_lleno.m — se aclara acá para evitar confusión, sin modificar
...
% Calculo tirante cr�tico
yc=min(d,(Q^2*(pi/4)^2/9.8)^(1/5));% Estimaciones inicales de tirante cr�tico
...
% Resoluci�n
par=[Q d S n];% vector con parametros 
options = odeset('Events',@(x,y) critico_lleno(x,y,yc,d));% para parar la resolucion de la ec. dif. cuando se llega al tirante cr�tico
...
p3=plot([x(1), x(end)],[yc-x(1)*S, yc-x(end)*S],'r');%zc cota de la superfice libre en flujo cr�tico
