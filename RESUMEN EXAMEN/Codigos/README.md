...
|---|---|---|
| `Bombas/` | `Bomba_sola.m`, `Bombas_serie.m`, `Bombas_paralelo.m`, `Bombas_LineasDeCarga.m`, `Bomba_curvaInstalacion.m`, `Bomba_manometros.m`, `Bomba_manometro_impulsion_paralelo.m`, `Bomba_bifurcacion.m`, `colebrook.m` | Sistemas de bombeo (succión + impulsión): punto de funcionamiento de una bomba sola, dos bombas en serie o en paralelo, línea de carga/piezométrica de la instalación, gráfico comparativo curva de instalación vs. curva de catálogo, punto de funcionamiento/NPSH cuando el dato son lecturas directas de manómetros en las bridas de la bomba (`Bomba_manometros.m`, sin necesidad de calcular pérdidas de tubería), punto de funcionamiento cuando sólo HAY UN manómetro aguas abajo de la(s) bomba(s) — succión por tubería normal desde un lago, y la cota del tanque final es la incógnita, no un dato (`Bomba_manometro_impulsion_paralelo.m`, admite 1 o varias bombas iguales en paralelo), y una bomba única que alimenta una red que se bifurca en N ramas idénticas aguas abajo (`Bomba_bifurcacion.m`, cada rama con Q/N). `Bombas_serie.m` trae los datos de 2023 Febrero Ej.4 precargados como ejemplo; sus vectores `eta1`/`eta2` tenían un bug (10 valores para 12 puntos de Q) corregido resolviendo ese examen — ver `resueltos/2023 Febrero/RESOLUCION.md` Ej.4. `Bomba_sola.m` trae precargados los datos exactos de 2018 diciembre Ej.4 (succión desde un lago + impulsión a un aspersor); ese examen se resolvió con una copia adaptada en `resueltos/2018 diciembre/Ejercicio4_riego_aspersor.m` que usa `fzero` (raíz exacta) en vez de la grilla de `Bomba_sola.m` para el punto de funcionamiento — necesario porque con una descarga por tobera/aspersor mucho más chica que la cañería, el término cinético de salida amplifica fuertemente cualquier redondeo de Q (ver `RESUMEN EXAMEN/Teorico/RESUMEN_TEORICO.md` §C1). Si un futuro examen tiene una tobera de salida angosta y pide una cota con precisión fina, preferir el patrón `fzero` de ese script en vez de la grilla de `Bomba_sola.m`. |
...
| `FGV_trapezoidal/` | `trap_geom.m`, `eq_yc.m`, `eq_yn.m`, `froude_trap.m`, `manning_trap.m`, `critico.m`, `rect.m` (ODE dy/dx, mal nombrado por herencia del template), `Mom_trap.m`, `Eesp_trap.m`, `alternos_trap.m`, `conjugados_trap.m`, `encontrar_resalto.m`, `caudal_M_ini.m`, `caudal_S_ini.m`, `descarga_ahogada.m`, `fgv_trap.m`, `tirantes_yn_yc.m`, `rasante_max.m`, `control_critico_lago_trap.m`, `sistema_lago_M.m`, `energia_trap_error.m`, `dos_lagos_trap.m` | Flujo Gradualmente Variado (FGV), tirante crítico/normal, energía específica y tirantes alternos, momento y tirantes conjugados, ubicación de resaltos hidráulicos, compuertas con descarga ahogada, tensión rasante máxima, caudal de control crítico de un lago que alimenta un tramo steep (`control_critico_lago_trap.m`, fzero anidado porque yc(Q) no es cerrado en trapezoidal) o un tramo **mild** muy largo (`sistema_lago_M.m`, sistema 2×2 energía+Manning para Q,yn), el tirante que da una energía específica objetivo (`energia_trap_error.m`, usado p.ej. para el tirante aguas arriba de un escalón que ahoga la sección), y el caudal de un canal **mild corto** (no "muy largo") entre dos lagos con ambos niveles conocidos, por shooting (`dos_lagos_trap.m`, con modo `'hLB_dado'` o `'critico_salida'` para hallar el Q máximo y el hLB umbral cuando Q deja de depender del lago de salida) — todo en canal **trapezoidal** (m=0 lo reduce a rectangular). |
...
| `Infiltracion/` | `horton_encharcamiento.py` | Python (no Octave/Excel — álgebra cerrada de Horton). Tiempo de encharcamiento de un evento en bloques, evolución de la tasa de infiltración real y volumen de escorrentía por balance simple, dado el modelo de Horton (f0, fc, k). Ver `RESUMEN EXAMEN/Teorico/RESUMEN_TEORICO.md` §B8, incluye el caveat de cuándo NO aplica la convención "una vez encharcado, siempre capacidad-limitado". |
| `Alcantarillas/` | `alcantarilla_tipo1.m`, `alcantarilla_tipo2.m` | Diseño/verificación de alcantarillas (Teórico HHA §3.2, clasificación de Bodhaine): balance de carga Tipo 1 (entrada y salida ahogadas) y Tipo 2 (entrada ahogada, sale a tubo lleno, "hidráulicamente larga"). Ver `README.md` propio de esa carpeta para la tabla de coeficientes de descarga y el criterio Tipo1 vs Tipo2 vs Tipo3. |
