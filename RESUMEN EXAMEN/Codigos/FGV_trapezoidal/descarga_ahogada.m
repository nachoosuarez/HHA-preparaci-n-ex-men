% descarga_ahogada.m — resuelve una COMPUERTA con DESCARGA AHOGADA
% (sumergida): se conserva energía entre la sección 1 (aguas arriba) y
% la 2 (bajo la compuerta, área fija dada por la apertura a), y se
% conserva MOMENTO entre la 2 y la 3 (aguas abajo, momento M3 dado/ya
% calculado). Resuelve el sistema 2x2 no lineal (energía + momento) con
% fsolve para hallar y2 (tirante inmediatamente aguas abajo de la
% compuerta) y E1 (energía específica aguas arriba).
...
% aguas abajo no permite salto libre) — para descarga LIBRE ver
% conjugados_trap.m / encontrar_resalto.m en su lugar. Requiere
% trap_geom.m.
function [y2_sol, E1_sol] = descarga_ahogada(M3, Q, b, m, a)
%se conserva la energ�a entre 1-2 y se conserva el momento entre 2-3
% Funci�n que resuelve el sistema de energ�a y momento para una compuerta
% con descarga ahogada, donde las inc�gnitas son:
%   y2 = tirante aguas abajo
%   E1 = energ�a espec�fica aguas arriba
%
% INPUTS:
...
% E1_sol energ�a espec�fica aguas arriba (m)

g = 9.8;

% �rea en movimiento bajo compuerta (fijo)
[~, Am, ~, ~, ~, ~] = trap_geom(a, b, m);
...
% Resoluci�n del sistema no lineal
...
% Geometr�a completa de la secci�n 2
