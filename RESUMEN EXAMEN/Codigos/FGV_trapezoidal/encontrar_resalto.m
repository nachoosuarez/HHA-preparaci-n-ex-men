% encontrar_resalto.m — ubica automáticamente un RESALTO HIDRÁULICO en
% un canal trapezoidal: calcula el perfil de FGV supercrítico (FGV1,
% aguas arriba) y el perfil subcrítico (FGV2, aguas abajo), calcula el
% CONJUGADO (vía Mom_trap.m) de cada punto de FGV1, y encuentra la
% intersección entre ese conjugado y la curva FGV2 — ese punto de
% intersección es la posición x del resalto. Grafica todo (FGV1, FGV2,
...
% condiciones iniciales de cada rama (x_ini/x_end/y_ini) según el
% problema. Usar cuando: se pide encontrar dónde ocurre el resalto en un
% canal con un tramo supercrítico seguido de uno subcrítico (o
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% RESALTO HIDR�ULICO EN CANAL TRAPEZOIDAL
% Visualizaci�n completa:
% - FGV1 (supercr�tico)
% - Conjugado de FGV1
% - FGV2 (subcr�tico)
% - L�neas de yc y yn
% - Intersecci�n entre conjugado(FGV1) y FGV2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(idx)
    disp('No hay intersecci�n entre el conjugado de FGV1 y FGV2');
    ...
    fprintf('\n>>> Intersecci�n (resalto) en x = %.4f m, y = %.4f m\n', x_inter, y_inter);
end
...
yline(yc, '--k', 'y_{cr�tico}', 'LineWidth', 1.5);
...
xlabel('x (m)');
ylabel('Tirante y (m)');
legend('FGV1 (supercr�tico)', 'Conjugado FGV1', 'FGV2 (subcr�tico)', ...
       'y_{cr�tico}', 'y_{normal}', 'Intersecci�n', 'Location', 'Best');
title('Intersecci�n entre FGV2 y el conjugado del flujo 1');
...
if ~isnan(x_inter)
    ...
    % 1) Punto en x donde se da la intersecci�n
    fprintf('\n1) La intersecci�n ocurre en x = %.4f m\n', x_inter);

    % 2) Tirante del flujo 1 en ese punto (FGV1)
    % Nos aseguramos que x_inter est� dentro del rango de x1
    if x_inter >= min(x1) && x_inter <= max(x1)
        y1_inter = interp1(x1, y1, x_inter, 'linear');
        fprintf('2) Tirante del flujo 1 en ese punto: y1 = %.4f m\n', y1_inter);
    else
        y1_inter = NaN;
        fprintf('2) x_inter est� fuera del rango de FGV1, no se puede interpolar y1.\n');
    end
    ...
    if x_inter >= min(x2) && x_inter <= max(x2)
        ...
        y2_inter = NaN;
        fprintf('3) x_inter est� fuera del rango de FGV2, no se puede interpolar y2.\n');
    end
    ...
    fprintf('\nNo se pudo calcular resultados adicionales porque no hubo intersecci�n.\n');
end
