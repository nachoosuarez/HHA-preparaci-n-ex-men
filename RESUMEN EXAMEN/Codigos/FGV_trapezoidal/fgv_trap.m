% fgv_trap.m — script principal de FLUJO GRADUALMENTE VARIADO (FGV) en
% canal TRAPEZOIDAL: calcula yc (crítico) y yn (normal), integra la
% ecuación diferencial dy/dx (función rect.m, pese al nombre) con
% ode23 deteniéndose al cruzar yc (evento critico.m), grafica el perfil
% de la superficie libre junto a las rasantes de yc/yn, y reporta si el
% perfil cruza yc o yn. Editar los DATOS DE ENTRADA (Q,b,S,n,m) y las
% condiciones de borde (x_ini,x_end,y_ini) según el enunciado — el signo
% de x_end define si se integra hacia aguas arriba (subcrítico) o hacia
% aguas abajo (supercrítico); ver comentario "NOTA" en el propio script.
% Usar cuando: piden el perfil y(x) de un canal trapezoidal (curvas
% M1/M2/M3, S1/S2/S3, etc.). Requiere trap_geom.m, rect.m, critico.m,
% froude_trap.m, manning_trap.m (el AVISO original de abajo menciona
% nombres de la versión rectangular por error de copia/pegado — los
% archivos correctos para esta versión trapezoidal son los recién
% listados).
...
par=[Q b m];% vector con par�metros
yc=fsolve(@(y) froude_trap(y,par), yc)% tirante critico (m)

[Bc,Ac,Pc,Rc,yGc,Dc]=trap_geom(yc,b,m);% c�lculo de par�metros geom�tricos asociados a las condiciones de flujo cr�tico
...
if S<=0
    ...
    par=[Q b S n m];% vector con par�metros 
    yn=fsolve(@(y) manning_trap(y,par), yn)% tirante normal (m)

    [Bn,An,Pn,Rn,yGn,Dn]=trap_geom(yn,b,m);% c�lculo de par�metros geom�tricos asociados a las condiciones de flujo normal
...
par=[Q b S n yc m];% vector con par�metros 
...
%figure(2)
%plot (x,tau)
%ylabel ('Tensi�n Rasante (MPa)','FontSize',14)
...
fprintf('Tirante cr�tico yc     = %.4f m\n',yc);
fprintf('Tirante normal yn      = %.4f m\n',yn);
fprintf('Energ�a cr�tica Ec     = %.4f m\n',Ec);
fprintf('Energ�a normal En      = %.4f m\n',En);
fprintf('Tirante final y_end    = %.4f m\n',y_end);
fprintf('---------------------------------------\n\n');
%==============================
% UBICACI�N DE CRUCE CON yc o yn
%==============================
tol = 1e-2;  % tolerancia num�rica

idx_yc = find(abs(y - yc) < tol, 1);
if ~isempty(idx_yc)
    fprintf('Cruce con tirante cr�tico yc en x = %.4f m\n', x(idx_yc));
end

idx_yn = find(abs(y - yn) < tol, 1);
if ~isempty(idx_yn)
    fprintf('Cruce con tirante normal yn en x = %.4f m\n', x(idx_yn));
end
