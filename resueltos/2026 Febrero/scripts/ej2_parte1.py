"""
Ejercicio 2, Parte 1 - Examen HHA 3/feb/2026
Caudal maximo de diseño de la alcantarilla, Tr=10 años.
...
de este examen (Tabla 1: Area=8.9 km2, dH=80m, L=3715m, Grupo D, S=5.7%).
"""
import numpy as np
import math
...
L_m = 3715.0
L_km = L_m/1000
S_basin_pct = 5.7      # pendiente media de la cuenca (uso: tabla C del metodo racional)
Grupo_Hidrologico = 'D'
Tr = 10                 # anios
P310 = 98.0             # mm, leido de isoyetas (Fig 3.1.10) en X=460km,Y=6620km (Artigas)
NC = 80                 # pastizales, condicion hidrologica buena, grupo D (Fig 3.1.20)
floor_rate = 1.2        # mm/h, infiltracion minima grupos B,C,D (grupo A = 2.4 mm/h)
C_racional = 0.38       # Pastizales, pendiente Promedio 2-7%, Tr=10 (Tabla 3.1.4)

# ---------- TIEMPO DE CONCENTRACION (Kirpich / Ramser-Kirpich) ----------
# S del canal principal (para Kirpich) = dH(m)/L(km)/10  -- DISTINTO de la
# pendiente media de la cuenca (5.7%) que se usa solo para la tabla de C.
...
# =====================================================================
# METODO RACIONAL  (tc entre 20min y 1h -> se calcula para comparar)
# =====================================================================
d_rac = tc_hs
CD_rac = CD(d_rac)
...
print(f"P(d,10,p) = {P_rac:.3f} mm ; i = {i_rac:.3f} mm/h")
...
deficit = np.maximum(N-Qinc, floor_rate*dt)        # piso de infiltracion (grupo D=1.2mm/h)
...
# malla temporal fina: paso tr/8 (igual que la planilla), suficientes puntos para cubrir
# la duracion total (11 pulsos*tr + Tb)
step = tr_uh/8
...
print(f"=> Se adopta el MAYOR (criterio del Teorico, 20min<tc<1h): "
      f"{'NRCS' if Qmax_NRCS>Q_racional else 'RACIONAL'}")
