"""
Ejercicio 2 - Examen HHA 11/dic/2023
Cuenca en Treinta y Tres (punto de cierre X=625km, Y=6350km), pastizales
naturales, Grupo Hidrologico B, condicion MALA, flujo concentrado.
Area=4.75 km2, dH=110 m, L=3100 m, S(media cuenca)=3.7%.
...
unitario triangular SCS), y resuelve las 3 partes del ejercicio:
  a) Qdiseño de la alcantarilla, Tr=5 anios.
  b) Tr del caudal limite de sobrepasamiento (Q=32 m3/s).
...
# Lecturas graficas/tabulares externas (Teorico HHA), tomadas de la
# solucion oficial manuscrita (coinciden con las figuras/tablas citadas):
P310 = 80.0        # mm, Figura 3.1.10 (isoyetas) en X=625km,Y=6350km (Treinta y Tres)
NC = 79             # pastizales naturales, condicion MALA, grupo B (Figura 3.1.20)
C_Tr5  = 0.36       # Tabla 3.1.4: pastizales/condicion mala/grupo B, S~3.7%, Tr=5
C_Tr10 = 0.38       # idem, Tr=10 (Tabla 3.1.4 tabula C por columnas discretas de Tr)

# ---------- TIEMPO DE CONCENTRACION (Kirpich / Ramser-Kirpich) ----------
S_channel_pct = dH_m / L_km / 10   # pendiente del CAUCE PRINCIPAL (≠ pendiente media 3.7%)
...
def metodo_racional(Tr, C):
    ...
    Q = C*i*Area_km2*100/360            # m3/s  (E22 = c*i*Area_Ha/360)
...
print("\n=================== PARTE B: Tr del caudal limite (Q=32 m3/s) ===================")
QMR_10, *_ = metodo_racional(10, C_Tr10)
print(f"Probando Tr=10 (Tabla 3.1.4: C={C_Tr10}): QMR(Tr=10) = {QMR_10:.2f} m3/s")
print(f"QMR(Tr=5)={QMR_5:.2f} < 32 m3/s < QMR(Tr=10)={QMR_10:.2f} m3/s")
print("=> Como la Tabla 3.1.4 tabula C en columnas DISCRETAS de Tr (2,5,10,25,50,100 años),")
print("   el caudal limite de 32 m3/s cae dentro del escalon de Tr=10: Tr_limite = 10 años")


print("\n=================== PARTE C: area maxima urbanizable ===================")
Qc_target = 1.15*Qa
C_target = Qc_target/Qa*C_Tr5    # Q es proporcional a C (i, A, tc fijos) => C_target = 1.15*C_Tr5
C_concreto = 0.80
A2 = Area_km2*(C_target - C_Tr5)/(C_concreto - C_Tr5)
print(f"Qtarget = 1.15*Qa = {Qc_target:.2f} m3/s")
print(f"C_target = 1.15*C_Tr5 = {C_target:.4f}  (Q prop. a C, con i,A,tc fijos)")
print(f"C_concreto/techo = {C_concreto}")
print(f"A2 (area urbanizable maxima) = AT*(C_target-C_natural)/(C_concreto-C_natural) = {A2:.3f} km2")

print("\n=====================================================")
print(f"RESUMEN: Qdiseño(Tr=5)={Qa:.2f} m3/s | Tr_limite(Q=32)=10 años | A_urbana_max={A2:.2f} km2")
